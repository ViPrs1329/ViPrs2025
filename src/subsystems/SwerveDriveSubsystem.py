...
class DriveTrain(commands2.Subsystem):
  ...
  def trigNormalize(self, givenJoyX, givenJoyY): 
    '''
    inputs direct joystick x and y values and outputs angle from standard position
    and magnitude. Does not account for shortest path. Can return angle and magnitude
    of 0 so make sure to account for that
    '''

    normVal = math.sqrt(givenJoyY ** 2 + givenJoyX ** 2) #the initial magnitude of inputs
    
    if normVal > 1: #if magnitude is more than 1, normalizes it
      joyY = givenJoyY / normVal
      joyX = givenJoyX / normVal

    else: #if magnitude is equal to or less than one, chill out
      joyY = givenJoyY
      joyX = givenJoyX
    
    yPos = abs(joyY) == joyY #boolean, y positivity
    xPos = abs(joyX) == joyX #boolean, x positivity
    
    #accounts for 0 magnitudes
    
    if joyY == 0: #on x axis
      m = abs(joyX)

      if xPos: #on pos x axis (or 0)
        t = 0

      else: #on neg x axis
        t = math.pi

    elif joyX == 0: #on y axis
      m = abs(joyY)
      
      if yPos: #on pos y axis (or 0)
        t = math.pi * 0.5
      ...
      t = math.atan2(joyY, joyX)
      m = math.sqrt(joyX ** 2 + joyY ** 2)

      if abs(t) != t: #math.atan can deliver negative values which are culled through this conditional, can remove if it doesn't matter
        t = math.pi * 2 + t

    return t, m #standard position angle, magnitude up to 1
  
  def deNormalize(self, t, m):
    # this can be changed
    maxModSpeed = 4.1
  ...
  def joystickFix(self, joyX, joyY):
    t, m = self.trigNormalize(joyX, joyY)
    return self.deNormalize(t, m)

  def driveFromChassisSpeeds(self, speeds: ChassisSpeeds) -> None:
    self.lastChassisSpeed = speeds

    # counter intuitive (should be this way plz don't change). This section is the original and has been commented out

    #Vx = speeds.vy
    #Vy = speeds.vx
    
    # this section is an implemented fix for swerve drive diagonal velocity errors. Comment out and uncomment the previous section to undo
    Vy, Vx = self.joystickFix(speeds.vx, speeds.vy)
  ...
  def manualDriveFromChassisSpeeds(self, speeds: ChassisSpeeds) -> None:
    ...
    #speeds = ChassisSpeeds(speeds.vx, -speeds.vy, -speeds.omega) #commented out as part of joystick fix
    speeds = ChassisSpeeds(Vy, -Vx, -speeds.omega) # part of joystick fix implementation
