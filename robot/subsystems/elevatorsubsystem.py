...
class ElevatorSubsystem(commands2.Subsystem):
    ...
    def __init__(self):
        ...
        motor2_conf = rev.SparkBaseConfig()
        motor1_conf.setIdleMode(rev.SparkBaseConfig.IdleMode.kBrake)
        motor2_conf.setIdleMode(rev.SparkBaseConfig.IdleMode.kBrake)
        
        # Set current limits
        # self.motor1.setSmartCurrentLimit(NEO_VORTEX_CURRENT_LIMIT)
        # self.motor2.setSmartCurrentLimit(NEO_VORTEX_CURRENT_LIMIT)
        motor1_conf.smartCurrentLimit(NEO_VORTEX_CURRENT_LIMIT)
        motor2_conf.smartCurrentLimit(NEO_VORTEX_CURRENT_LIMIT)

        self.motor1.configure(motor1_conf, reset_mode, persist_mode)
        self.motor2.configure(motor2_conf, reset_mode, persist_mode)
        
        # Create absolute encoder
        self.encoder = self.motor1.getAbsoluteEncoder()
        
        # Configure encoder
        # There is no way to configure the encoder.
        ...
        # encoder_conf = rev.AbsoluteEncoderConfig()
        # encoder_conf.positionConversionFactor(1.0)
